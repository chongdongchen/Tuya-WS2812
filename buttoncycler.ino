...
unsigned char pid[] = {"ypdqrlctsfytcbpn"};//*********处替换成涂鸦IoT平台自己创建的产品的PID
...
unsigned char dp_process(unsigned char dpid,const unsigned char value[], unsigned short length)
{
  switch(dpid) {
    case DPID_SWITCH_LED:
      ...
      if (dp_bool_value) {
        //Turn on
        colorfill (strip.Color(  50, 50,  50)); //上一次状态
        strip.show();
      } else {
        //Turn off
    ...
    dp_enum_value  = my_device.mcu_get_dp_download_data(dpid, value, length); /* Get the value of the down DP command */
      switch(dp_enum_value){
        ...
        break;
        case 1: // colour mode
          colorfill (strip.Color(  100, 0,  0));
        break;
        case 2: // scene mode
          colorfill (strip.Color(  0, 100,  0));
        break;
       case 3: // music mode
          colorfill (strip.Color(  0, 0,  100));
        break;
